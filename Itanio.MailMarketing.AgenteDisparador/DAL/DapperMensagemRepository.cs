...
namespace Itanio.MailMarketing.AgenteDisparador
{
    public class DapperMensagemRepository : IMensagemRepository
    {
        ...
        private readonly string selectContatos = @"select 
                              ema.ema_id                         Id
        ...
        private readonly string selectContatosJaEnviados = @"select
                                                ema.ema_id                          IdEmail
                                                , ema.mai_id                         IdMailing
                                                , ema.ema_nome                       Nome
                                                , ema.ema_email                      Email
        ...
        private readonly string selectEmpresas = @"select 
	                                     EMP.codigo                 Id
        ...
        private readonly string selectMailings = @"select 
                              mai.mai_id                         Id 
        ...
        private readonly string selectMensagem = @"
                        select
                            msg.msg_id                           Id 
                            , msg.msg_nome                       Nome
        ...
        private readonly string selectNoticias = @"select
                             con.codigo Id 
                            , con.Secao Secao
                            , con.Titulo Titulo
                            , con.figura Figura
        ...
        private readonly string selectSegmentos = @"
                        select
                            seg.seg_id                          Id 
                            , seg.seg_nome                       Nome
                ...
                new SqlConnection(ConfigurationManager.ConnectionStrings["revista_infra"].ConnectionString))
            {
                                                               ...
                                                               + selectContatosJaEnviados, new {Id = idMensagem}))
                {
                    ...
                    foreach (var mailingsPorSegmento in mailings.GroupBy(x => x.IdSegmento))
                        mensagem.Segmentos.Find(x => x.Id == mailingsPorSegmento.Key).Mailings
                            .AddRange(mailingsPorSegmento.ToList());

                    var contatos = resultados.Read<Contato>();
                    foreach (var contatosPorMailings in contatos.GroupBy(x => x.IdMailing))
                        mensagem.Segmentos.SelectMany(x => x.Mailings).Single(x => x.Id == contatosPorMailings.Key)
                            .Contatos.AddRange(contatosPorMailings.ToList());
        ...
        public int InserirLink(int idMensagem, string link)
        {
                ...
                new SqlConnection(ConfigurationManager.ConnectionStrings["revista_infra"].ConnectionString))
            {
                ...
                return _conexao
                    .Query<int>(
                        "INSERT INTO upm_link(MSG_ID, LIN_URL, LIN_STATUS) values (@Id, @Link, 1); SELECT SCOPE_IDENTITY() ",
                        new {Link = link, Id = idMensagem}).Single();
            }
        }
        ...
        public void InserirEnvioInfo(EnvioInfo envioInfo)
        {
                ...
                new SqlConnection(ConfigurationManager.ConnectionStrings["revista_infra"].ConnectionString))
            {
                _conexao.Open();
                _conexao.Execute(
                    "INSERT INTO upm_msgemail_email(msg_id,ema_id,mem_dataEnvio,mem_status, mem_erro) values (@IdMensagem, @IdContato, getdate(), @Status, @Erro) ",
                    envioInfo);
            }
        }
        ...
        public void Atualizar(Mensagem mensagem)
        {
                ...
                new SqlConnection(ConfigurationManager.ConnectionStrings["revista_infra"].ConnectionString))
            {
                _conexao.Open();
                _conexao.Execute(
                    "UPDATE upm_msgemail set statusEnvio = @status, msg_inicioEnvio = @inicioEnvio, msg_fimEnvio = @fimEnvio where msg_id = @id ",
                    new
                    {
